...
The `--tag-override` CLI flag overrides `tag` / `tag_if` in affected recipes. It accepts a mapping file, a scalar tag for one recipe, or multiple inline mappings.

A single value ending in `.lock`, `.yaml`, `.yml`, or `.json` is loaded as a mapping file from package names to git refs:
...
forest grow my_package --tag-override overrides.json
```

Any other single value is used as the git ref for the one selected recipe:

```bash
forest grow my_package --tag-override devel
```

When passing multiple values, each value must use `pkg:=tag` syntax:

```bash
forest grow my_package --tag-override dep_a:=main dep_b:=v1.2.3
```
