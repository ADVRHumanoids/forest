...
forest --add-recipes git@github.com:MarcoRuzzon/forest-recipes.git gripper-sim 
forest all -j$(nproc) --debug-pwd user
# expect -f $HOME/scripts/forest_all.expect $(nproc)

forest --add-recipes git@github.com:MarcoRuzzon/forest-recipes.git robots
forest ModularBot_conda_gripper -j$(nproc) 
...
roslaunch ModularBot_conda_gripper ModularBot_gazebo.launch verbose:=true paused:=false&
roslaunch_pid="$!"
sleep 2

xbot2-core --hw sim --verbose&
xbot2_core_pid="$!"
sleep 2

rosservice call xbotcore/gripper/switch 1
