...
echo user | sudo -Sk cp -f xenomai.conf /etc/ld.so.conf.d/
echo user | sudo -Sk ldconfig
...
forest all -j$(nproc) --mode xeno -v --debug-pwd user
# expect -f $HOME/scripts/forest_all_xeno.expect $(nproc)
